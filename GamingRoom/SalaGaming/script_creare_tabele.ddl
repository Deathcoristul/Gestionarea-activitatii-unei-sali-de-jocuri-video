...
ALTER TABLE detalii_jucator ADD CONSTRAINT "E-mail_UN" UNIQUE ( email );
...
CREATE TABLE serviciu_streaming (
    nume        VARCHAR2(40) NOT NULL,
    id_serviciu NUMBER(8) NOT NULL
);

ALTER TABLE serviciu_streaming ADD CONSTRAINT serviciu_streaming_pk PRIMARY KEY ( id_serviciu );
...
ALTER TABLE sesiune_gaming
    ADD CONSTRAINT streaming_fk FOREIGN KEY ( id_serviciu )
        REFERENCES serviciu_streaming ( id_serviciu );

ALTER TABLE sesiune_gaming
    ADD CONSTRAINT tip_joc_fk FOREIGN KEY ( id_tip )
...
CREATE SEQUENCE serviciu_streaming_id_serviciu START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER serviciu_streaming_id_serviciu BEFORE
    INSERT ON serviciu_streaming
    FOR EACH ROW
    WHEN ( new.id_serviciu IS NULL )
BEGIN
    :new.id_serviciu := serviciu_streaming_id_serviciu.nextval;
END;
/
